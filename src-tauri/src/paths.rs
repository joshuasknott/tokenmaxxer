...
/// Root for isolated Codex CLI profiles. Each child directory is named with a
/// UUID generated by TokenMaxxer, so cleanup never follows a user-supplied path.
pub fn codex_profiles_dir() -> io::Result<PathBuf> {
    Ok(app_data_dir()?.join("codex-profiles"))
}
