...
| Amazon Bedrock | AWS access key/secret/session token with CloudWatch permissions | CloudWatch `InputTokenCount`, `OutputTokenCount`, cache token counts, and throttles from `AWS/Bedrock`. | Cost is not derived in v1 adapter; AWS Cost Explorer can be a later enhancement. | CloudWatch retention/region behavior applies. | AWS IAM credentials are highly sensitive; use least privilege. | Implemented through signed CloudWatch `GetMetricData`. |
| Azure OpenAI / AI Foundry | Azure management bearer token plus resource id | Azure Monitor `ProcessedPromptTokens`, `GeneratedTokens`, and `TokenTransaction`. | Cost is not derived in v1 adapter; Azure Cost Management can be a later enhancement. | Azure Monitor retention and resource metric availability apply. | Management token grants Azure resource metadata access; keep local only. | Implemented. |
| Fireworks AI | Metrics CSV path or authenticated `firectl` | Official `firectl billing export-metrics` CSV includes prompt/completion token columns. | Cost is not derived in v1 adapter because the export is token/usage focused. | Depends on local `firectl` when not using CSV import. | Exported CSV can include project/model metadata. | Implemented as CSV/export-backed provider. |
