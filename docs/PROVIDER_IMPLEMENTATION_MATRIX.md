...
| P1 | xAI / Grok | Ship for teams | Management API exposes team billing balance and usage endpoints. Requires a management key plus team id; normal model keys are not enough. |
| P1 | Amazon Bedrock | Ship for AWS users | CloudWatch publishes official Bedrock token metrics under `AWS/Bedrock`. Requires IAM credentials with `cloudwatch:GetMetricData`. |
| P1 | Azure OpenAI / AI Foundry | Ship for Azure users | Azure Monitor publishes resource metrics for processed prompt tokens, generated tokens, and token transactions. Requires an Azure management bearer token. |
| P1 | Fireworks AI | Ship as export-backed | Official `firectl billing export-metrics` produces billing CSV rows with prompt/completion token fields. API-key-only billing API not used. |
...
- xAI / Grok: team prepaid balance and best-effort usage totals from Management API billing endpoints.
- Amazon Bedrock: CloudWatch token/throttle metrics for the `AWS/Bedrock` namespace.
- Azure OpenAI / AI Foundry: Azure Monitor token metrics for a supplied resource id.
...
| xAI / Grok | Management API key plus `team_id` | `/v1/billing/teams/{team_id}/usage` exposes historical usage. Adapter treats usage as best effort and still reports balance if usage shape changes. | `/v1/billing/teams/{team_id}/prepaid/balance` reports prepaid balance. | Management API authorization and team scope. | Team-level billing data and spend limits. | Implemented. |
| Amazon Bedrock | AWS access key/secret/session token with CloudWatch permissions | CloudWatch `InputTokenCount`, `OutputTokenCount`, cache token counts, and throttles from `AWS/Bedrock`. | Optional `estimated_cost_gbp`/`estimated_cost_usd` credential field for reconciled period cost. AWS Cost Explorer can be a later enhancement. | CloudWatch retention/region behavior applies. | AWS IAM credentials are highly sensitive; use least privilege. | Implemented through signed CloudWatch `GetMetricData`. |
| Azure OpenAI / AI Foundry | Azure management bearer token plus resource id | Azure Monitor `ProcessedPromptTokens`, `GeneratedTokens`, and `TokenTransaction`. | Optional `estimated_cost_gbp`/`estimated_cost_usd` credential field for reconciled period cost. Azure Cost Management can be a later enhancement. | Azure Monitor retention and resource metric availability apply. | Management token grants Azure resource metadata access; keep local only. | Implemented. |
| Fireworks AI | Metrics CSV path or authenticated `firectl` | Official `firectl billing export-metrics` CSV includes prompt/completion token columns. | Reads `cost`, `cost_usd`, or `cost_gbp` columns when present; otherwise accepts cost override fields. | Depends on local `firectl` when not using CSV import. | Exported CSV can include project/model metadata. | Implemented as CSV/export-backed provider. |
...
7. xAI / Grok: Management API team balance/usage.
8. Amazon Bedrock: CloudWatch token metrics.
9. Azure OpenAI / AI Foundry: Azure Monitor token metrics.
10. Fireworks AI: firectl billing metrics export.
11. Gemini API, Mistral, Together: keep in backlog until official usage/balance endpoints are available or verified.
...
- OpenAI, Anthropic, Claude Code, Cursor, Contextual AI, xAI, Bedrock, Azure OpenAI, and Fireworks produce historical bucketed usage. They may need an additional `UsageSeries`/history-first rendering path instead of forcing everything into reset windows.
...
- xAI Management API billing: https://docs.x.ai/docs/api-reference#billing
- AWS Bedrock CloudWatch metrics: https://docs.aws.amazon.com/bedrock/latest/userguide/monitoring-cw.html
- Azure Monitor supported metrics for Microsoft.CognitiveServices/accounts: https://learn.microsoft.com/azure/azure-monitor/reference/supported-metrics/microsoft-cognitiveservices-accounts-metrics
- Fireworks firectl billing metrics export: https://docs.fireworks.ai/tools-sdks/firectl/firectl_commands/billing
