...
export function AddAccountWizard({
  ...
  async function submit() {
    ...
    try {
      ...
      await addAccount(label.trim() || defaultLabel(provider), provider, creds);
      setStep("done");
      onAdded();
    } catch (e) {
  ...
  async function completeCodexSetup(profileId: string) {
    setStep("working");
    try {
      await completeCodexProfile(profileId, label.trim() || defaultLabel("codex"));
      setStep("done");
      onAdded();
    } catch (e) {
      setErrorMsg(friendlyError(String(e)));
        ...
        {(step === "credentials") && provider && (
          <div className="space-y-4">
            ...
            <CredentialInstructions
              ...
              onCodexComplete={completeCodexSetup}
            />
...
interface CredentialInstructionsProps {
  ...
  showSecret: boolean;
  onToggleSecret: () => void;
  onCodexComplete: (profileId: string) => Promise<void>;
}
...
function CredentialInstructions({
  ...
  if (provider === "codex") {
    return <CodexProfileSetup onComplete={onCodexComplete} />;
  }
...
function CodexProfileSetup({
  ...
  onComplete: (profileId: string) => Promise<void>;
}) {
